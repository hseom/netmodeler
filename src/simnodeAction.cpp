...
void QueryAction::Execute() {
  ...
  while (ni->moveNext() ) {
    ...
    //cout << query_ignore << endl;
    //if((inNode->getprob()<query_ignore && inNode->token_check(node->getAddress())) || (inNode->getprob()<query_ignore && gotoniche<0.1)) {
    if(inNode->getprob()<query_ignore && inNode->token_check(node->getAddress())) {
        //node->token_increase(inNode->getAddress());
        //cout << 'a' << endl;
        //cout << inNode->getAddress() << "\t" << query_ignore << endl;
